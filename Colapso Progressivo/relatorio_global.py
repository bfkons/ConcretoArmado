...
def adicionar_relatorio(pilar_ref: str, relatorio_texto: str) -> bool:
    """
    Adiciona relatório ao JSON temporário

    Args:
        pilar_ref: Referência do pilar (ex: 'P24')
...
def gerar_relatorio_global_texto() -> Optional[str]:
    ...
    linhas = []
    linhas.append("=" * 80)
    linhas.append(" " * 20 + "RELATORIO GLOBAL - COLAPSO PROGRESSIVO")
    linhas.append(" " * 30 + "NBR 6118:2023 - Item 19.5.4")
    linhas.append("=" * 80)
    linhas.append(f"Sessao iniciada em: {dados['data_inicio_sessao']}")
    linhas.append(f"Total de pilares verificados: {len(dados['relatorios'])}")
    linhas.append("=" * 80)
    linhas.append("")
...
def salvar_relatorio_global_txt(caminho_saida: Optional[str] = None) -> Optional[str]:
    ...
    if caminho_saida is None:
        ...
        nome_arquivo = f"relatorio_colapso_{timestamp}.txt"
